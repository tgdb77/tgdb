...
- 或可明確拆成「一次性本機建置映像（`.build`）+ 固定 runtime 單元」
...
- 容器映像：名稱與 tag 建議
- 若上游要求自行 build：來源倉庫/版本、Dockerfile/Containerfile 路徑、build context、產出 tag
- 容器內埠號與對外埠建議
- 必要環境變數（required）與可選變數
- 需要持久化的路徑（Volume）
- 首次初始化/後置任務需求（migrate/seed/admin init 等）

若上游不是直接提供可用 image，而是要求本機 build，先確認：
- build 是否可在部署時自動完成，而不是要求使用者手動先跑 `podman build`
- build context 是否能穩定準備（例如 clone repo、下載 release source、複製本地檔）
- 上游 Dockerfile 是否需要為 Podman/Buildah 做相容性修補（短名映像、`COPY --link`、build args 等）
- 產出的 image tag 是否能以實例名區分，避免多實例互相覆蓋
...
若 App 需要 build，通常還要新增：
- `config/<service>/quadlet/default.build`
- `config/<service>/scripts/pre_build_*.sh`
- `config/<service>/scripts/post_build_*.sh`
...
- `var=`（例如 secret）
- `config=`（設定檔模板）
- `pre_build=` / `post_build=`（需要 `.build` 時）
...
### 3.1.1 Build 型 App 的實際做法（目前可參考 `misskey`）

TGDB 目前已有 `.build` 流程，已知 AppSpec 實例是 `config/misskey/`。實作原則：
- `app.spec` 的 `image=` 先寫最終目標映像，例如 `localhost/tgdb/<service>:latest`
- `quadlet_type` 使用 `multi`
- 用 `unit=build|template=quadlet/default.build|suffix=.build` 宣告一次性建置單元
- runtime `.container` 改引用 build 產出的 tag，通常用實例名區分，例如 `localhost/tgdb/<service>:${container_name}`
- 若 build 前需要抓原始碼/改寫 Dockerfile，就用 `pre_build=...`
- 若 build 完要清掉 `/tmp` context，就用 `post_build=...`

TGDB 部署 build 單元時的已知行為：
- 先 render 所有 unit，再檢查 `.build` 的 `ImageTag=`
- 若本機已存在對應 tag，預設會略過建置
- 若映像不存在，會先執行 `pre_build`
- 接著啟動 `.build` Quadlet 進行 `podman build`
- build 成功後執行 `post_build`
- 最後移除 runtime 的 `.build` unit，只保留已建好的 image 與其他服務單元

因此技能判斷時，若需求只是「部署時順手 build 一次 image 再跑 container」，**仍可優先考慮 AppSpec**，不必直接打回進階應用。

### 3.1.2 `default.build` 模板建議

`.build` 模板至少應明確設定：
- `[Build] File=`：Dockerfile/Containerfile 路徑
- `[Build] SetWorkingDirectory=`：build context 目錄
- `[Build] ImageTag=`：建置產物 tag（建議含 `${container_name}`，避免多實例衝突）

建議做法：
- build context 優先放在可預期的暫存路徑，例如 `/tmp/tgdb-build-<service>-${container_name}`
- 若來源需要下載/clone，路徑白名單要寫死前綴，避免誤刪任意目錄
- 若上游 Dockerfile 使用短名映像，必要時在 `pre_build` 改寫成完整名稱（例如 `docker.io/library/node:`）
- 若上游 Dockerfile 含 Podman 相容性問題，集中在 `pre_build` 修補，不要把修補散落在主流程
- `post_build` 只做清理，不要承擔部署成功與否的關鍵邏輯
...
若本次 App 有 `.build`，再額外確認：
- render 後有產生 `${container_name}.build`
- `.build` 內的 `ImageTag=` 與 runtime `.container` 的 `Image=` 一致對上同一命名規則
- `pre_build` / `post_build` 腳本只使用相對於 `config/<service>/` 的路徑
- 若 `pre_build` 會抓遠端 source，需確認失敗訊息清楚、重跑可重現

...
_tgdb_ts_fwd_chain_ensure() {
    ...
    # 重要：若要 DNAT 到 127.0.0.1，通常需要允許從 tailscale0 路由到 localnet（127/8）。
    # 否則可能會被內核視為 martian packet 而丟棄，外部表現多為 timeout/連不上。
    ...
    if [ "$need_allow_tcp" -eq 1 ] || [ "$need_allow_udp" -eq 1 ]; then
        ...
        local h_reject
        h_reject=$(echo "$in_list" | awk '/reject with icmpx type admin-prohibited/ {for(i=1;i<=NF;i++){if($i=="handle"){print $(i+1); exit}}}')
        if [ -n "${h_reject:-}" ]; then
    ...
    # nat prerouting chain + redirect rules
...
nftables_ts_forward_show() {
    ...
    echo "----------------------------------"
    echo "說明：當其他節點連到本機 tailscale0 的 <port> 時，會被 redirect 到 127.0.0.1:<port>。"

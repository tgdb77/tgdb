...
| 🎯 功能類別 | 📝 具體說明 |
|:----------:|:----------|
| **容器化應用** | 內建 200+ 個 App Spec，涵蓋媒體、AI、知識管理、監控、資料庫與網路服務（預設 rootless；部分應用支援 rootful） |
| **安全堆疊** | nftables 防火牆 + Fail2ban 入侵防護，完整安全防護 |
| **自動備份** | 全備份/還原、指定實例備份/還原、Kopia 熱備、Rclone 遠端同步與拉回、systemd timer 排程、已備份管理 |
...
> 💡 **注意**
>
> - TGDB 會在首次互動模式啟動時，嘗試自動建立預設快捷鍵 `t`（`~/.local/bin/t`）；後續可透過 `9. 快捷鍵管理` 維護。
> - 建立快捷鍵時，TGDB 會嘗試自動把 `~/.local/bin` 加入你的 shell `PATH`；若目前 shell 尚未重新載入設定，可能要重新登入、開新終端，或手動執行 `source ~/.bashrc` 才會立即生效。
> - 首次使用若涉及安裝套件、調整系統設定或防火牆規則，會需要輸入 `sudo` 密碼。
...
══════════════════════════════════════════
 ...
 8. 第三方腳本      → 實用的第三方腳本
 9. 快捷鍵管理      → 自訂執行快捷鍵
10. 全系統備份管理  → 全備份 / 指定實例備份 / 還原 / 自動備份 / 已備份管理 / Kopia
11. 定時任務管理    → 備份 / DB / Nginx / 自訂 timer / Healthchecks
12. 服務觀測        → 查看 TGDB 相關 systemd 狀態與 journal 日誌
══════════════════════════════════════════
777. 快速環境設定    → 快速設定新環境
...
- 預設備份範圍以：
  ...
  - Quadlet units
  為主
- `volume_dir` 預設**不納入**主備份內容；若 App 使用 `volume_dir`，全還原與指定還原後都會自動補建必要目錄與 `volume_subdirs`，避免因缺少目錄結構而啟動失敗。

---

## 📱 支援的應用

TGDB 使用 **Podman + Quadlet** 實現 **rootless / rootful** 容器化部署（預設 rootless），目前內建 **200+ 個 App Spec**：
...
- 主選單 12 提供 TGDB 相關 **systemd / Quadlet / timer** 的統一觀測入口
- 目前支援：
  - 查看服務狀態詳情
  - 查看服務日誌
  - 即時追蹤日誌
  - 重新啟動後查看最後 50 行日誌
- 預設以 **TGDB 管理範圍內的單元** 為主，不是全系統服務總管
- 目前以 **互動選單模式** 為主
...
- `volume_dir` 資料內容預設不納入 TGDB 備份；但全還原與指定還原後，若 App 有使用 `volume_dir`，TGDB 會自動補建必要目錄與 `volume_subdirs`
